...
class MalariaFullPipeline:
    ...
    def __init__(self, yolo_weights, clf_checkpoint, yolo_conf=0.25, yolo_iou=0.45, clf_batch_size=64, device=None):
    ...
    def _classify_crops(self, crops):
        ...
        for i in range(0, len(crops), self.clf_batch_size):
            ...
            tensors = torch.stack([self.clf_transform(img) for img in batch_pil]).to(self.device)
    ...
    def visualize(self, image_path, results, save_path=None, show=False):
        ...
        for r in results:
            ...
            cv2.putText(img, label, (x1 + 1, y1 - 3), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 0), 1)
